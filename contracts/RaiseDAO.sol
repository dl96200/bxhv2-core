...
contract RaiseDAO is ReentrancyGuard,Ownable {
  ...
  function harvestVWhen( uint256 blocknumber,address userAddr) public view returns (uint256 offeringTokenAmount,uint256 currentAmount,uint256 refundingTokenAmount) {
    ...
    if(!user.claimed){
      offeringTokenAmount = getOfferingAmount(blocknumber,userAddr);
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    return getOfferingAmount(block.number,_user);
  }

  function getOfferingAmount(uint blocknumber, address _user) public view returns(uint256) {
    (uint256 offeringAmountCurrent, ) = getReleaseUpdate(block.number);
    if (totalAmount > raisingAmount) {
      uint256 allocation = getUserAllocation(_user);
      return offeringAmountCurrent.mul(allocation).div(1e6);
    }
    else {
      // userInfo[_user] / (raisingAmount / offeringAmount)
      return userInfo[_user].amount.mul(offeringAmountCurrent).div(raisingAmount);
    }
  }
