...
contract UniswapV2Factory is IUniswapV2Factory ,Ownable{
    ...
    function getReferAmount(uint256 amount) public override view returns (uint256){
        return amount.wdiv(10000).wmul(referRatio);
    }

    function rewardUpper(address ref,address token,uint256 amount) public override  {
        if(refers!=address(0x0)){
            TransferHelper.safeTransfer(token,refers,amount);
            IReferences(refers).rewardUpper(ref,token,amount,feeTo);
        }
    }
