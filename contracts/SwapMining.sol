...
contract SwapMining is Ownable {
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
     ...
     function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        //share to upper
         uint256 upperFee = factory.getReferAmount(_amount);
        if(upperFee>0)
        {
            _amount = _amount.sub(upperFee);
            bxh.transfer(address(factory), upperFee);
            factory.rewardUpper(_to,address(bxh),upperFee);         
        }
